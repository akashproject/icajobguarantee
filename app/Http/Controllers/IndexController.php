...
class IndexController extends Controller
{
    ...
    public function weabinerCaptureLead(Request $request){
        ...
        try {
            ...
            $data =[];
            $mail = Mail::send('email.webinarLeadCaptureTemplate', $data, function ($m) use ($postData) {
                $m->from('connect@icajobguarantee.com', 'ICA Edu Skils');
                $m->to($postData['email'], $postData['name'])->subject("Confirmation and Joining Instructions for Stock Market Webinar on 23rd January’24");
            });
            //$this->sendEmailBrochure($postData);
    ...
    public function classroomLeadCaptureLeadToExtraage($postData){
        ...
        $apiData = array(
            ...
            'Remarks' => (isset($postData['occupation']))?$postData['occupation']:'',
        );
