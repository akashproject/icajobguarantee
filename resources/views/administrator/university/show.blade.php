...
<div class="col-12">
	<div class="card">
		<form class="form-horizontal" method="post" action="{{ url('administrator/save-university') }}" enctype="multipart/form-data">
			@csrf
			<div class="card-body">
				...
				<div class="row">
					...
					<div class="col-md-5">
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<select name="state_id" id="state_id" class="select2 form-control custom-select" style="width: 100%; height:36px;" onChange="getCitiesByStateId(this);" >	
									...
									<option value="{{ $value->id }}" {{ ( $university->state_id ==  $value->id )? 'selected' : '' }} > {{ $value->name }} </option>
									@endforeach
								<select>
							</div>
						</div>
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<select name="city_id" id="city_id" class="select2 form-control custom-select" style="width: 100%; height:36px;">	
									...
									<option value="{{ $value->id }}" {{ ( $university->city_id ==  $value->id )? 'selected' : '' }} > {{ $value->name }} </option>
									@endforeach
								<select>
							</div>
						</div>					
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<select name="enable_otp" id="enable_otp" class="select2 form-control custom-select" style="width: 100%; height:36px;">	
									...
									<option value="1" {{ ( $university->enable_otp ==  '1' )? 'selected' : '' }}> Yes</option>
									<option value="0" {{ ( $university->enable_otp ==  '0' )? 'selected' : '' }}> No </option>
								<select>
							</div>
						</div>
						<div class="form-group row">
							...
							<div class="col-sm-9">
								<select name="status" id="status" class="select2 form-control custom-select" style="width: 100%; height:36px;">	
									...
									<option value="1" {{ ( $university->status ==  '1' )? 'selected' : '' }} > Publish</option>
						...
						<div class="form-group row">
							...
							<div class="col-sm-8 text-center">
								<a href="#imageBox" class="image-profile open-popup-link">
									<img src="{{ (isset($university->featured_image))?getSizedImage('',$university->featured_image):'https://dummyimage.com/250x250?text=Add%20Image' }}" alt="" style="width:100%">
						...
						<div class="form-group row">
							...
							<div class="col-sm-8 text-center">
								<a href="#imageBox" class="image-profile open-popup-link">
									<img src="{{ (isset($university->banner_image))?getSizedImage('',$university->banner_image):'https://dummyimage.com/250x250?text=Add%20Image' }}" alt="" style="width:100%">
									<input type="hidden" name="banner_image" id="banner_image" value="{{ $university->banner_image }}" >	
								</a>	
								@if(isset($university->banner_image))
									<a href="javascript:void(0)" class="removeImage" style="color: #c90f0f;font-weight: 600;"> Remove Image </a>	
								@endif					
							</div>
						</div>
