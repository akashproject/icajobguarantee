...
<div class="col-12">
	<div class="card">
		<form class="form-horizontal" method="post" action="{{ url('administrator/save-university-ad-page') }}" enctype="multipart/form-data">
			@csrf
			<div class="card-body">
				...
				<div class="row">
					...
					<div class="col-md-5">
						<div class="form-group row">
							<label for="university_id" class="col-sm-3 text-right control-label col-form-label">Courses</label>
							<div class="col-sm-9">
								<select name="university_id" id="university_id" class="select2 form-control custom-select">	
									<option value="">Select University</option>
									@foreach($universities as $university)
									<option value="{{ $university->id }}" {{ ($university->id == $adPage->university_id)?'selected':'' }}>{{ $university->name }}</option>
									@endforeach
								<select>
							</div>
						</div>
